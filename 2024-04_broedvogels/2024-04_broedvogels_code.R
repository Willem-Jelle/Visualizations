...
codes_broedvogels <- read_csv2("2024-04_broedvogels_data_broedvogels.csv") |>
  clean_names() |>
  select(identifier_broedvogels = identifier,
         naam_broedvogel = title)

codes_perioden <- read_csv2("2024-04_broedvogels_data_perioden.csv") |>
  clean_names() |>
  select(identifier_perioden = identifier,
         jaar = title)

observations_broedvogels <- read_csv2("2024-04_broedvogels_data_observations.csv") |>
  clean_names() |>
...
trend_broedvogels <- observations_broedvogels |>
  ...
  mutate(trend = case_when(
    ...
    jaar == 2022 & naam_broedvogel == "Gierzwaluw" ~ "toegenomen",
    jaar == 2022 & naam_broedvogel == "Rode Wouw" ~ "toegenomen",
    jaar == 2022 & trendcijfer >= 100 ~ "toegenomen",
    jaar == 2022 & trendcijfer < 100 ~ "afgenomen",
    .default = NA)) |>
  tidyr::fill(trend,
              .direction = "up") |>
...
for(i in 1:n_pages(gg_viz)){
  ...
  ggsave(plot = gg_save,
         filename = paste0("2024-04_broedvogels_viz_0",
                           i,
                           ".png"),
         width = 7500,
         height = 7500,
